                ...
                <div className="col-md-8">
                    {
                        dataGradeBook && dataGradeBook?.lessons?.map((result1, index1) => {
                            return <div key={index1} className="grader-unit mb-40">
                                ...
                                {
                                    result1.topics?.map((result2, index2) => {
                                        return <div key={index2} className="body-grader bg-neutral-50 radius-4 px-24 py-8" style={styleShadow}>
                                            <table className="w-100 my-8">
                                                <thead>
                                                    <tr>
                                                        <th className="font-sm-bold py-16 text-start">No</th>
                                                        <th width="80%" className="font-sm-bold py-16 text-start">{result2?.name}</th>
                                                        <th className="font-sm-bold py-16 text-center">Grade</th>
                                                    </tr>
                                                </thead>
                                                <tbody>
                                                    {
                                                        result2.subtopics.length > 0 ?
                                                            result2.subtopics?.map((item, index) => {
                                                                let totalScore = 0
                                                                return <tr key={index} className="border-top border-secondary-500">
                                                                    ...
                                                                    <td className="py-16 font-sm text-center">                                                                        
                                                                        <>
                                                                            {
                                                                                item.timelines?.map((res, i) => {
                                                                                    if (res.grades.length > 0 && !res.grades[0].score) {
                                                                                        totalScore = 1000000
                                                                                    }
                                                                                    if (res.grades.length > 0 && res.grades[0].score) {
                                                                                        if(res.grades[0].score > totalScore) {
                                                                                            totalScore = res.grades[0].score
                                                                                        }
                                                                                    }
                                                                                })
                                                                            }
                                                                            { totalScore < 1000000 ? totalScore.toFixed(1) : 'On grading' }
                                                                        </>
                                                                    </td>
                                                                </tr>
                                                            })
