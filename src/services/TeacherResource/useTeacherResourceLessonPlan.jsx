...
function useTeacherResourceLessonPlan() {
    ...
    const initData = (dataPage, dataPerPage, dataKeyword, dataFilter) => {
        setIsLoading(true)
        setData([])
        GET(`/resources/lesson_plan?page=${dataPage}&per_page=${dataPerPage}&keyword=${dataKeyword}`, defConfig(dataFilter))
            .then(
                (res) => {
                    setData(res.data)
                    setPagination({
...
export default useTeacherResourceLessonPlan